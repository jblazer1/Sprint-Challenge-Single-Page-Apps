...
> Answer: One of the benefits of client side routing is that we can maintain state or memory within out application and use that memory to display to display the resources requested. Another benefit is that the page won't refresh. The data is there and displayed when the user asks for it.
...
> Answer: Controlled components are components that have the input and output driven from one source of truth. This is done by giving the component value from state.
